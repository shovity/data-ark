...
test('every backup gets a row read from its summary card', async () => {
  ...
  assert.match(out.text(), /ark-20260905-7f3a91 +data\.tar +21\.4 GB +12 +2026-09-05/)
  assert.match(out.text(), /ark-20260901-9de447 +photos\.zip +940\.3 MB +1 +2026-09-01/)
})
...
test('the footer counts the backups and shows how to restore one', async () => {
  ...
  assert.match(out.text(), /npx data-ark restore <backup-id>/)
})
...
test('a manifest without a summary card is listed with what the message itself knows', async () => {
  ...
  const old = {
    id: 1800,
    fileName: 'ark-20260820-aa11bb.manifest.json',
    caption: '#dataark ark-20260820-aa11bb manifest',
    date: Math.floor(Date.parse('2026-08-20T09:00:00.000Z') / 1000),
  }

  await runList({}, deps(configDir, [old], out))

  assert.match(out.text(), /ark-20260820-aa11bb +— +— +— +2026-08-20/)
})
...
test('messages that are not manifests are left out', async () => {
  ...
  const chunk = {
    id: 1500,
    fileName: 'ark-20260905-7f3a91.part0003',
    caption: '📦 ark-20260905-7f3a91 · 3/12',
...
// The file name is data-ark's own, the caption is text anyone in the chat can edit.
test('the backup id comes from the file name, not from the caption', async () => {
  const configDir = await workspace()
  const out = collect()
  const edited = {
    ...DATA_TAR,
    caption: DATA_TAR.caption.replace('ark-20260905-7f3a91', 'ark-tampered-000000'),
  }

  await runList({}, deps(configDir, [edited], out))

  assert.match(out.text(), /ark-20260905-7f3a91/)
  assert.doesNotMatch(out.text(), /ark-tampered-000000/)
})
