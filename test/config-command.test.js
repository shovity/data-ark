...
// The listing shows keys data-ark does not know so a typo is visible from the command
...
test('a setting data-ark does not know is shown, and can be removed', async () => {
  const configDir = await tempDir('config')
  await saveConfig({ settings: { chat: 'me', chukSize: '500MB' } }, configDir)
  const shown = collect()

  await runConfig([], {}, deps(configDir, shown))
  assert.match(shown.text(), /Ignored, data-ark does not know these: chukSize/)
